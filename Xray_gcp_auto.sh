...
DEFAULT_SOCKS_USERNAME="darrenwu"                #默认socks账号
DEFAULT_SOCKS_PASSWORD="admin+1S"                #默认socks密码
...
config_xray() {
	...
	START_PORT=$DEFAULT_START_PORT  # 使用默认端口而不是用户输入
	if [ "$config_type" == "socks" ]; then
		SOCKS_USERNAME=$DEFAULT_SOCKS_USERNAME  # 使用默认SOCKS用户名
		SOCKS_PASSWORD=$DEFAULT_SOCKS_PASSWORD  # 使用默认SOCKS密码
	elif [ "$config_type" == "vmess" ]; then
		UUID=$DEFAULT_UUID  # 使用默认生成的UUID
		WS_PATH=$DEFAULT_WS_PATH  # 使用默认的WebSocket路径
	fi
